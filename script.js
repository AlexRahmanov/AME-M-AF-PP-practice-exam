let examChoice="AF", currentIndex=0, activeQuestions=[], responses=[], timerId=null, remainingSeconds=null, startedAt=null;
const $=id=>document.getElementById(id); const screens=["startScreen","quizScreen","resultsScreen","reviewScreen"];
...
function render(){const q=activeQuestions[currentIndex],r=responses[currentIndex],total=activeQuestions.length;$("progressText").textContent=`${currentIndex+1} / ${total}`;$("progressFill").style.width=`${(currentIndex+1)/total*100}%`;$("questionNumber").textContent=`${q.bank} • ${q.id}`;$("questionTopic").textContent=`${q.section} ${q.topic}`;$("questionText").textContent=q.question;stats();drawTimer();const box=$("options");box.innerHTML='';q.options.forEach((text,i)=>{const b=document.createElement('button');b.type='button';b.className='option';b.innerHTML=`<span class="letter">${String.fromCharCode(65+i)}.</span><span>${esc(text)}</span>`;if(r){if($("quizMode").value==='practice'){b.disabled=true;if(i===q.answer)b.classList.add('reveal');if(i===r.choice)b.classList.add(r.correct?'correct':'wrong');}else if(i===r.choice)b.classList.add('selected');}b.onclick=()=>answer(i);box.appendChild(b)});const fb=$("feedback");fb.className='feedback hidden';fb.innerHTML='';if(r&&$("quizMode").value==='practice')feedback(q,r);$("prevBtn").disabled=currentIndex===0;$("nextBtn").textContent=currentIndex===total-1?'Finish Quiz':'Next Question';}
